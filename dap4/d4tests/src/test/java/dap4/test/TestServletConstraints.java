...
public class TestServletConstraints extends DapTestCommon {
  ...
  static protected class TestCase {
    ...
    protected TestCase(int id, String dataset, String extensions, String ce, Dump.Commands template) {
      this(id, dataset, extensions, ce, true, template);
    }

    protected TestCase(int id, String dataset, String extensions, String ce, boolean checksumming,
  ...
  @Test
  public void testServletConstraints() throws Exception {
    DapCache.flush();
  ...
  protected void defineAllTestcases() {
    this.alltestcases.add(new TestCase(1, "test_one_vararray.nc", "dmr,dap", "/t[1]",
        // S4
        new Dump.Commands() {
    ...
    this.alltestcases.add(new TestCase(2, "test_anon_dim.syn", "dmr,dap", "/vu32[0:3]", // test for dimension inclusion
        // S4
        new Dump.Commands() {
          public void run(Dump printer) throws IOException {
            printer.startchecksum();
    ...
    this.alltestcases.add(new TestCase(3, "test_one_vararray.nc", "dmr,dap", "/t", // test for dimension inclusion
        // S4
        new Dump.Commands() {
          public void run(Dump printer) throws IOException {
            printer.startchecksum();
    ...
    this.alltestcases.add(new TestCase(4, "test_enum_array.nc", "dmr,dap", "/primary_cloud[1:2:4]",
        // 2 S1
        new Dump.Commands() {
          public void run(Dump printer) throws IOException {
            printer.startchecksum();
    ...
    this.alltestcases.add(new TestCase(5, "test_atomic_array.nc", "dmr,dap",
        "/vu8[1][0:2:2];/vd[1];/vs[1][0];/vo[0][1]", new Dump.Commands() {
          public void run(Dump printer) throws IOException {
            printer.startchecksum();
            for (int i = 0; i < 2; i++) {
              printer.printvalue('U', 1, i);
    ...
    this.alltestcases.add(new TestCase(6, "test_struct_array.nc", "dmr,dap", "/s[0:2:3][0:1]", new Dump.Commands() {
      public void run(Dump printer) throws IOException {
        printer.startchecksum();
        for (int i = 0; i < 4; i++) {
          for (int j = 0; j < 2; j++) {
    ...
    this.alltestcases.add(new TestCase(7, "test_opaque_array.nc", "dmr,dap", "/vo2[1][0:1]", new Dump.Commands() {
      public void run(Dump printer) throws IOException {
        printer.startchecksum();
        for (int i = 0; i < 2; i++) {
          printer.printvalue('O', 0, i);
    ...
    this.alltestcases.add(new TestCase(8, "test_atomic_array.nc", "dmr,dap", "/v16[0:1,3]", new Dump.Commands() {
      public void run(Dump printer) throws IOException {
        printer.startchecksum();
        for (int i = 0; i < 3; i++) {
    ...
    this.alltestcases.add(new TestCase(9, "test_atomic_array.nc", "dmr,dap", "/v16[3,0:1]", new Dump.Commands() {
      public void run(Dump printer) throws IOException {
        printer.startchecksum();
        for (int i = 0; i < 3; i++) {
          printer.printvalue('S', 2, i);
  ...
  List<TestCase> locate(Object tag) {
